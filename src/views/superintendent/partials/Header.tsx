...
function Header() {
	...
	const myProfile = useQuery(["myProfile"], () => {
		...
		return AuthService.me({ isNotify: false });
	});
	...
	if (myProfile?.data?.data?.roles?.name != "SUPERINTENDENT") {
		cookiesHandler.deleteCookie("access_token");
		navigation.replace("/auth/signin");
	}

	return (
		<div className="flex flex-col md:flex-row justify-between gap-8 mb-4 px-1">
